# Vendored from ohmyzsh/plugins/git-commit.
# LOCAL CHANGE: $ZSH -> $ZSH_CONFIG_DIR — omz uses its install dir's HEAD as a
# version marker to avoid rewriting gitconfig every start; we use the dotfiles
# repo HEAD instead. Everything below this line is verbatim.

local _rev="$(git -C "$ZSH_CONFIG_DIR" rev-parse HEAD 2> /dev/null)"
